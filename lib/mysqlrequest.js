...
class RestMySqlRequest {
  ...
  async find(req, res, next) {  
    ...
    let orderby = ""
    if (req.get("dblimit")) {
      limit = parseInt(req.get("dblimit"))
      if (isNaN(limit)) limit = 200
    }
    if (req.get("dborder")) {
      let flds = req.get("dborder").split(',')
      for(let i=0; i < flds.length;i++) {
        let parts = flds[i].split(' ')
        parts[0] = req.db.pool.escapeId(parts[0].trim())
        flds[i] = parts.join(' ')
      }
      orderby = ` ORDER BY ${flds.join(',')} `
    }

    let sql = null 
    if (req.db.name) {
      sql = `SELECT * FROM ${req.db.pool.escapeId(req.db.name)}.${req.db.pool.escapeId(this.table)} WHERE ${fields.join(" AND ")} ${orderby} LIMIT ${limit} `
    } else {
      sql = `SELECT * FROM ${req.db.pool.escapeId(this.table)} WHERE ${fields.join(" AND ")} ${orderby} LIMIT ${limit} `
    }
  ...
  static create(tables) {
    ...
    for(let table of tables) {
      results.push(new RestMySqlRequest(table))
    }
    return results
  }
...
module.exports = RestMySqlRequest