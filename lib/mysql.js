...
// fetch primarykey
async function getIdentityKeys(pool, table, dbname) {
  ...
  if (!cache || cache.expires < Date.now()) {
    ...
    cache = { expires: Date.now()+EXPIRES_IN, keys: [] }
...
// fetch primarykey
async function getPrimaryKeys(pool, table, dbname) {
  let key = dbname ? `${dbname}.${table}` : table
...
// fetch fields 
async function getFields(pool, table, dbname) {
  let key = dbname ? `${dbname}.${table}` : table
  ...
  if (!cache || cache.expires < Date.now()) {
    ...
    cache = { expires: Date.now()+EXPIRES_IN, fields: [] }
...
class RestMySql {
  ...
  async get(req, res, next) {  
    if (!req.db || !req.db.pool) throw new TypeError("A database pool was not found (req.db.pool)")
    let pkeys = await getPrimaryKeys(req.db.pool, this.table, req.db.name)
  ...
  async add(req, res, next) {  
    if (!req.db || !req.db.pool) throw new TypeError("A database pool was not found (req.db.pool)")
    let idents = await getIdentityKeys(req.db.pool, this.table, req.db.name)
    let columns = await getFields(req.db.pool, this.table, req.db.name)
  ...
  async save(req, res, next){  
    if (!req.db || !req.db.pool) throw new TypeError("A database pool was not found (req.db.pool)")
    let pkeys = new Set(await getPrimaryKeys(req.db.pool, this.table, req.db.name))
    let idents = new Set(await getIdentityKeys(req.db.pool, this.table, req.db.name))
    let columns = await getFields(req.db.pool, this.table, req.db.name)
